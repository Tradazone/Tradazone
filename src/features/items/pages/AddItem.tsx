...
import { useCurrencyPreference, FIAT_SYMBOLS } from '../../../utils/currencyPreference';
...
function AddItem() {
    ...
    const priceLabel = currencySymbol
        ? `Price (${displayCurrency} ${currencySymbol})`
        : `Price (${displayCurrency})`;

    const handleSubmit = (e) => {
        e.preventDefault();
        addItem({ ...formData, currency: displayCurrency });
        navigate('/items');
    };
            ...
            <form onSubmit={handleSubmit} className="bg-white border border-border rounded-card p-6">
                <div className="flex flex-col gap-5 mb-6">
                    ...
                    <p className="text-xs text-t-muted -mt-2">
                        Prices are shown in <strong>{displayCurrency}</strong>. Change your display currency in{' '}
                        <Link to="/settings/payments" className="text-brand hover:underline">
                            Settings → Payments
                        </Link>.
                    </p>
                </div>
